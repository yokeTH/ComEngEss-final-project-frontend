...
export async function createTag() {
  ...
  const nameTag = document.querySelector(".add-tag").value;

  if (nameTag !== "" && tags.find((e) => e.nameTag === nameTag) === undefined) {
    tags.push({ name: nameTag, score });
    drawTable();
  }
}
...
export async function post() {
  ...
  if (topicName !== "" && description !== "" && imageUrl !== "") {
    await createPost({ topicName, tags, description, image: url });
